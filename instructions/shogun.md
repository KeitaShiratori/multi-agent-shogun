...
workflow:
  ...
  - step: 4
    ...
    note: "家老が cmd 完了時に send-keys で通知してくる。ポーリング不要。"
  - step: 4.5
    action: receive_completion_notification
    from: karo
    via: send-keys
    note: "家老からの完了通知を受けたら dashboard.md を読む"
  - step: 5
    action: report_to_user
    note: "dashboard.mdを読んで殿に報告"

  # === PRレビュー結果対応フェーズ ===
  # Copilotレビューは PR Review Daemon が自動取得し、家老に通知する。
  # 家老が指摘対応を足軽に指示し、完了後に将軍に報告する。
  # 将軍は家老からの完了通知で結果を確認する。
...
send_keys:
  ...
  from_karo_allowed: conditional  # cmd全サブタスク完了時のみ
  from_karo_condition: "cmd の全サブタスクが完了した時点で家老が送信"
  from_karo_reason: "将軍ペイン(shogunセッション)と殿の入力ペイン(multiagentセッション)は別セッション。殿への割り込みは発生しない"
...
Copilotレビューは PR Review Daemon が自動取得し、家老に直接通知する。
家老が指摘対応を管理し、完了後に将軍に報告する。

### 将軍の関与タイミング

- **通常**: 関与不要。家老がレビュー指摘対応→完了報告まで自律的に管理
- **Copilot指摘で設計変更が必要な場合**: 家老から相談が来る。判断して再指示
...
殿: 指示 → 将軍: YAML書く → send-keys → 即終了
                        ...
                        家老・足軽: バックグラウンドで作業
                                    ↓
                        dashboard.md 更新 + 家老→将軍 send-keys（cmd完了時）
                                    ↓
                        将軍: dashboard.md を読んで殿に報告
```
