...
# 結果を queue/pr/done/ のサブディレクトリに振り分け、tmux send-keys で家老に通知する。
#
# done/ サブディレクトリ:
#   done/mergeable/  - レビュー指摘なし（マージ可能）
#   done/needs-fix/  - レビュー指摘あり（修正必要）
#   done/merged/     - マージ完了
#
# Usage:
...
#   ./scripts/pr_review_daemon.sh --daemon     # デーモンモード（内部使用）
...
ensure_dirs() {
    ...
    mkdir -p "$DONE_DIR/mergeable" "$DONE_DIR/needs-fix" "$DONE_DIR/merged"
...
# ═══════════════════════════════════════════════════════════════════════════════
# YAML配列値取得（depends_on: [4, 5] → "4 5"）
# ═══════════════════════════════════════════════════════════════════════════════
yaml_get_array() {
    local file="$1"
    local key="$2"
    grep "^${key}:" "$file" 2>/dev/null | sed "s/^${key}: *//; s/\[//; s/\]//; s/,/ /g; s/\"//g; s/'//g" | tr -s ' '
}
...
process_pr() {
    ...
    if [ -z "$pr_number" ] || [ -z "$repo" ]; then
        ...
        mv "$yaml_file" "$DONE_DIR/needs-fix/${filename%.yaml}_error.yaml"
        return 1
    fi
    ...
    if [ "$has_review" = true ]; then
        ...
        if [ "$total_comments" -gt 0 ]; then
            done_subdir="needs-fix"
            status_label="Copilotレビュー指摘${total_comments}件あり"
        else
            done_subdir="mergeable"
            status_label="Copilotレビュー指摘なし（マージ可能）"
        fi

        local done_file="$DONE_DIR/${done_subdir}/$filename"

        # depends_on があれば引き継ぎ
        local depends_on_line
        depends_on_line=$(grep "^depends_on:" "$inprogress_file" 2>/dev/null || true)
...
DONE_EOF
        log "INFO: Copilot review found for ${repo}#${pr_number} (${total_comments} comments) → done/${done_subdir}/"
    else
        # Copilotレビューなし（タイムアウト）→ マージ可能扱い
        done_subdir="mergeable"
        status_label="Copilotレビューなし（タイムアウト・マージ可能）"
...
DONE_EOF
        log "INFO: No Copilot review for ${repo}#${pr_number} (timeout after ${COPILOT_TIMEOUT}s) → done/${done_subdir}/"
    fi
    ...
    # 家老に通知
    if [ -n "$notify" ]; then
        notify_karo "$notify" "PR#${pr_number}（${cmd_id}）の${status_label}。queue/pr/done/${done_subdir}/${filename} を確認せよ。"
    fi
}
...
# ═══════════════════════════════════════════════════════════════════════════════
merge_mergeable_prs() {
    for yaml_file in "$DONE_DIR/mergeable"/*.yaml; do
        [ -f "$yaml_file" ] || continue

        local filename
        filename=$(basename "$yaml_file")
        local pr_number repo notify cmd_id
        pr_number=$(yaml_get "$yaml_file" "pr_number")
        repo=$(yaml_get "$yaml_file" "repo")
        notify=$(yaml_get "$yaml_file" "notify")
        cmd_id=$(yaml_get "$yaml_file" "cmd_id")
        ...
        if [ -z "$pr_number" ] || [ -z "$repo" ]; then
            log "WARN: Skipping invalid mergeable YAML: ${filename}"
            continue
        fi

        # depends_on チェック
        local depends_on
        depends_on=$(yaml_get_array "$yaml_file" "depends_on")
        local all_deps_merged=true

        if [ -n "$depends_on" ]; then
            for dep_pr in $depends_on; do
                dep_pr=$(echo "$dep_pr" | tr -d '[:space:]')
                [ -z "$dep_pr" ] && continue

                # 依存先PRが done/merged/ に存在するか確認
                local dep_found=false
                for merged_file in "$DONE_DIR/merged"/*.yaml; do
                    [ -f "$merged_file" ] || continue
                    local merged_pr
                    merged_pr=$(yaml_get "$merged_file" "pr_number")
                    if [ "$merged_pr" = "$dep_pr" ]; then
                        dep_found=true
                        break
                    fi
                done
                ...
                if [ "$dep_found" = false ]; then
                    ...
                    log "INFO: PR#${pr_number} depends on PR#${dep_pr} which is not yet merged. Skipping."
                    break
                fi
            done
        fi

        # 依存先未マージ → スキップ（次のサイクルで再チェック）
        if [ "$all_deps_merged" = false ]; then
            continue
        fi

        # マージ実行
        log "INFO: Merging PR#${pr_number} (${repo})..."
        local merge_result
        if merge_result=$(gh pr merge "$pr_number" --repo "$repo" --merge 2>&1); then
            # マージ成功
            log "INFO: PR#${pr_number} merged successfully"

            # YAML に status: merged を追記
            echo "status: merged" >> "$yaml_file"
            echo "merged_at: \"$(date '+%Y-%m-%dT%H:%M:%S')\"" >> "$yaml_file"

            # done/merged/ に移動
            mv "$yaml_file" "$DONE_DIR/merged/$filename"

            # 家老に通知
            if [ -n "$notify" ]; then
                notify_karo "$notify" "PR#${pr_number}（${cmd_id}）マージ完了。done/merged/${filename} を確認せよ。"
            fi
        else
            # マージ失敗
            log "ERROR: Failed to merge PR#${pr_number}: ${merge_result}"

            # YAML に status: merge_failed を追記
            echo "status: merge_failed" >> "$yaml_file"
            echo "merge_error: \"${merge_result}\"" >> "$yaml_file"

            # done/needs-fix/ に移動
            mv "$yaml_file" "$DONE_DIR/needs-fix/$filename"

            # 家老に通知
            if [ -n "$notify" ]; then
                notify_karo "$notify" "PR#${pr_number}（${cmd_id}）マージ失敗。done/needs-fix/${filename} を確認せよ。"
            fi
        fi
    done
}
...
cmd_status() {
    if is_running; then
        ...
        echo "  done/mergeable: $(ls "$DONE_DIR/mergeable"/*.yaml 2>/dev/null | wc -l) files"
        echo "  done/needs-fix: $(ls "$DONE_DIR/needs-fix"/*.yaml 2>/dev/null | wc -l) files"
        echo "  done/merged:    $(ls "$DONE_DIR/merged"/*.yaml 2>/dev/null | wc -l) files"
        echo ""
...
case "${1:-}" in
    ...
    --daemon)
        ...
        ;;
    *)
        echo "Usage: $0 {start|stop|restart|status|--foreground|--daemon}"
        echo ""
        echo "Commands:"
        ...
        echo "  --daemon     Internal daemon mode (used by start)"
        exit 1
        ;;
esac
