...
1. **自分のIDを確認**: `tmux display-message -t "$TMUX_PANE" -p '#{@agent_id}'`
   - `shogun` → 将軍
   - `karo1`, `karo2`, `karo3` → 家老1, 家老2, 家老3
   - `ashigaru1` ～ `ashigaru6` → 足軽1～6
...
└──────┬───────┘
       ...
       ▼
┌──────────────┬──────────────┬──────────────┐
│    KARO1     │    KARO2     │    KARO3     │ ← 家老×3（タスク指示・スループット最大化）
│   (家老1)    │   (家老2)    │   (家老3)    │   各2足軽を統率
└──────┬───────┴──────┬───────┴──────┬───────┘
       │              │              │
       ▼              ▼              ▼
┌───┬───┐      ┌───┬───┐      ┌───┬───┐
│A1 │A2 │      │A3 │A4 │      │A5 │A6 │ ← 足軽×6（実働部隊）
└───┴───┘      └───┴───┘      └───┴───┘
       │              │              │
       └──────────────┼──────────────┘
                      │ 成果物完成・PR作成
                      ▼
              ┌──────────────┐
              │  PR Review   │ ← Daemon（Copilotレビュー自動取得）
              │   Daemon     │
              └──────┬───────┘
                     │ レビュー結果を家老に通知
                     ▼
...
| 役職 | 人数 | 役割 | 専念すべきこと |
|------|------|------|----------------|
| 将軍 | 1 | プロジェクト統括 | 殿との対話、戦略判断 |
| 家老 | 3 | タスク管理・分配・PR作成 | 足軽への指示出し、スループット最大化（各2足軽を統率） |
| 足軽 | 6 | 実作業 | コード実装、ドキュメント作成 |
| PR Review Daemon | 1 | Copilotレビュー自動取得 | バックグラウンドでPRレビュー監視 |
...
### 報告の流れ（割り込み防止設計 + PRレビュー自動化）

**タスク実行フロー:**
- **将軍→家老**: YAML + send-keys で指示
- **家老→足軽**: YAML + send-keys で指示
- **足軽→家老**: 報告YAML記入 + send-keys で報告

**PRレビューフロー（Daemon連携）:**
```
足軽: コード修正 → コミット → プッシュ → 家老に報告
家老: PR作成（テンプレート使用）→ queue/pr/ready/ にYAML配置 → 次タスクへ
Daemon: Copilotレビュー取得 → queue/pr/done/ → 家老に通知
家老: 指摘あり → 足軽に修正指示 / 指摘なし → 将軍に完了報告
```

**dashboard.md更新:**
- 家老のみが更新（単一責任）
- 将軍へのsend-keysは cmd 全完了時のみ許可

- 理由: 殿（人間）の入力中に割り込みが発生するのを防ぐ。同一tmuxセッション内の通信は割り込みリスクなし
...
queue/pr/ready/                   # 家老がPR作成後にYAML配置 → Daemon監視
queue/pr/inprogress/              # Daemon処理中
queue/pr/done/                    # Daemonがレビュー結果出力 → 家老が確認
scripts/pr_review_daemon.sh       # PR Review Daemon本体
...
```
┌───────────┬───────────┬───────────┐
│   karo1   │   karo2   │   karo3   │  ← Pane 0, 3, 6
│  (家老1)  │  (家老2)  │  (家老3)  │
├───────────┼───────────┼───────────┤
│ ashigaru1 │ ashigaru3 │ ashigaru5 │  ← Pane 1, 4, 7
│  (足軽1)  │  (足軽3)  │  (足軽5)  │
├───────────┼───────────┼───────────┤
│ ashigaru2 │ ashigaru4 │ ashigaru6 │  ← Pane 2, 5, 8
│  (足軽2)  │  (足軽4)  │  (足軽6)  │
└───────────┴───────────┴───────────┘
```

- Pane 0: karo1（家老1）
- Pane 1-2: ashigaru1-2（足軽、家老1の配下）
- Pane 3: karo2（家老2）
- Pane 4-5: ashigaru3-4（足軽、家老2の配下）
- Pane 6: karo3（家老3）
- Pane 7-8: ashigaru5-6（足軽、家老3の配下）
...
- instructions/bugyo.md - **廃止**（歴史的参照用として保持）
...
### 3.5. 家老からの完了通知
- 家老は cmd の全サブタスクが完了した時点で、将軍に send-keys で通知する
- 通知を受けたら dashboard.md を読んで状況を把握し、殿に報告する
- 個別の足軽完了時には通知は来ない（dashboard.md で確認）
